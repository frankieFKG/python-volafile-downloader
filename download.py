    ...
    def download_room(self):
        ...
        for f in file_list:
            ...
            if not self.max_file_size == -1 and f.size / 1048576 >= self.max_file_size:
                ...
                print(' !! ERR: File got filtered out.')
        print('### ### ###')
    ...
    def single_file_download(self, url, upl):
        ...
        if os.path.isfile(download_path):
            ...
            if local == online:
                ...
                print(" !! ERR: Found Diff File Size!")
                print("         " + str(local) + " vs " + str(online))
                #return False;
                fileNameCount = 0
                fileExists = True
                while (fileExists):
                    download_path = temp_path + str(file_split[0]) + "-(" + str(fileNameCount) + ').' + str(file_split[-1]) #VolaDL.id_generator()
                    fileExists = os.path.isfile(download_path)
                    if ( fileExists ):
                        local = os.path.getsize(download_path)
                        if ( local == online ):
                            print( "     Found a correct sized version" )
                            return True
        ...
        success = False;
        while ( not success ):
            success = self.download_file(url, download_path)
            attempts -= 1
            if ( attempts <= 0 ):
                print ( " !! ERR: out of attempts for file " + url );
                return False
            ...
            if os.path.isfile(download_path):
                ...
                online = self.savedSize
                if local != online:
                    print (" !! ERR: file failed to download somehow" )
                    print ("         " + str(local) + " vs " + str(online))
                    success = False
                else:
